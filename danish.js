...
const https = require('https'); // ✅ NEW: GitHub approval fetch ke liye
const http = require('http');   // ✅ NEW: keep-alive ping ke liye
...
const approvalFilePath = path.join(process.cwd(), 'https://raw.githubusercontent.com/waleedali98776-oss/VPs-server-wp/refs/heads/main/approval.txt%E2%80%89');

// ========== ✅ NEW: GITHUB APPROVAL SYSTEM CONFIG ==========
const configFilePath = path.join(process.cwd(), 'config.json');
const APPROVAL_REFRESH_MS = parseInt(process.env.APPROVAL_REFRESH_MS || '60000', 10); // har 60 sec sync
...
// ✅ NEW: Fetch text from any URL (GitHub raw file)
...
// ✅ NEW: Sync approved keys from GitHub approval.txt
async function syncGithubApprovals() {
  const cfg = loadConfig();
  const url = (process.env.APPROVAL_GITHUB_URL || cfg.approvalUrl || '').trim();
  if (!url) return { synced: false, count: 0, reason: 'No approval URL set' };
  try {
    ...
    logger.info(`🐙 GitHub approvals synced: ${keys.length} keys`);
...
// ✅ UPDATED: Check if key is approved (local file OR GitHub)
function isKeyApproved(key) {
  const k = String(key || '').trim().toUpperCase();
  if (!k) return false;
  if (GITHUB_APPROVED_KEYS.has(k)) return true; // ✅ GitHub check pehle
  const approvedKeys = loadApprovedKeys().map(x => String(x).trim().toUpperCase());
  return approvedKeys.includes(k);
}
...
// Always-on sender loop (supports message/image/sticker/video)
...
// ========== ✅ NEW: GITHUB APPROVAL ADMIN ROUTES ==========

// Admin — Set GitHub approval URL
...
// Admin — Get approval config/status
app.post('/api/admin/approval-config', (req, res) => {
  try {
    ...
    res.json({
      ok: true,
      url: process.env.APPROVAL_GITHUB_URL || cfg.approvalUrl || '',
...
// Admin — Force sync GitHub approvals now
...
// ========== ✅ UPDATED: Initialize ==========
initializeUsers();
syncGithubApprovals();                                 // ✅ GitHub sync on server start
...
// ✅ NEW: Keep-alive self ping (free hosting ke liye, set KEEP_ALIVE=true)
